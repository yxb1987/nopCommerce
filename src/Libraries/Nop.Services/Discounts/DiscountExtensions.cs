...
namespace Nop.Services.Discounts
{
    public static class DiscountExtensions
    {
        ...
        public static DiscountPrice GetDiscountPrice(this IList<DiscountForCaching> allowedDiscounts, DiscountPriceRequest requestDetails)
        {
            ...
            result.DiscountPricesByQuantity = appliedDiscounts.Select(discount => discount.DiscountQuantity).Distinct()
                ...
                }).OrderBy(priceByQuantity => priceByQuantity.Quantity).ToList();
            result.PriceWithDiscount = result.DiscountPricesByQuantity.First(priceByQuantity => priceByQuantity.Quantity == requestDetails.Quantity).DiscountPrice;
